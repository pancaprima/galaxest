...
def run(test_suite, adb_ids, opts=None, background=False):
    if framework == automations_const.KEY_FRAMEWORK_KATALON:
        adb_ids_array = adb_ids.split(',')
        for adb_id in adb_ids_array :
            katalon.run(config.data.katalon_app, config.data.katalon_project_path, config.data.katalon_project_file, test_suite, adb_id, opts, background)
    else :
...
def is_using_device(local_id):
    ...
    proc = subprocess.Popen("ps xu | grep '%s' | grep -v grep | awk '{ print $2 }'" % (identifier), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
